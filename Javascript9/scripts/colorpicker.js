const setup = () => {
    ...
    let r = localStorage.getItem("R.colorValue");
    let g = localStorage.getItem("G.colorValue");
    let b = localStorage.getItem("B.colorValue");
    colorDemos[0].style.backgroundColor="rgb(" + r +"," + g + "," + b +")";

    document.getElementsByClassName("slider")[0].value = r;
    document.getElementsByClassName("slider")[1].value = g;
    document.getElementsByClassName("slider")[2].value = b;
...
const update = () => {
    ...
    localStorage.setItem("R.colorValue", sliders[0].value);
    localStorage.setItem("G.colorValue", sliders[1].value);
    localStorage.setItem("B.colorValue", sliders[2].value);
...
const saveColor = () => {
    ...
    localStorage.setItem("savedcolor"+document.getElementsByClassName("colorDiv").length, color);
    localStorage.setItem("savedColors.amount",document.getElementsByClassName("colorDiv").length);
}

const previouslySavedColours = () => {
    for (let i = 1; i <= localStorage.getItem('savedColors.amount'); i++) {
